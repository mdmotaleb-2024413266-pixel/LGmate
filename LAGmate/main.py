...
import asyncio
import edge_tts
import nest_asyncio  # FIX: Handles Event Loop issues
...
# --- CONFIGURATION ---
GROQ_API_KEY = os.getenv("GROQ_API_KEY", "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")
...
# --- VOICE GENERATION (ENGLISH ONLY) ---
async def generate_voice_base64(text):
    try:
        # Strictly using English Voice
...
def chat():
    ...
    # Internal Prompt - Strict English Tutor
    default_instruction = (
    ...
    try:
        ...
        # Audio Generation (Using asyncio.run with nest_asyncio fix)
        try:
            # nest_asyncio allows us to safely use asyncio.run inside Flask
            audio_data = asyncio.run(generate_voice_base64(ai_reply))
        except Exception as audio_err:
            print(f"Audio Gen Error: {audio_err}")
            audio_data = None
